...
public class ExampleJobController {
    ...
    @PostMapping("/run")
    @Operation(summary = "Run Example Batch Job", description = "Triggers the example batch job to process raw data")
    public ResponseEntity<String> runBatch() {
        try {
            exampleJobService.runJob();
            return ResponseEntity.ok("Example batch job has been triggered successfully");
        } catch (Exception e) {
            log.error("Error triggering example batch job", e);
            return ResponseEntity.internalServerError().body("Error triggering example batch job: " + e.getMessage());
        }
    }
}